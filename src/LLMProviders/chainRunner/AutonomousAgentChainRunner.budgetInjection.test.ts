...
describe("buildBudgetNote", () => {
  ...
  describe("decrements by 1 per fetchWebPages call", () => {
    it("reflects each successive decrement", () => {
      ...
      expect(buildBudgetNote(remainders[0], MAX_PARALLEL)).toContain("3 fetch attempt(s) remaining");
      expect(buildBudgetNote(remainders[1], MAX_PARALLEL)).toContain("2 fetch attempt(s) remaining");
